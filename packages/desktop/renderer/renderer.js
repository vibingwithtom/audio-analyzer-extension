...
class DesktopAudioAnalyzer {
  ...
  attachEventListeners() {
    ...
    if (window.electronAPI) {
      window.electronAPI.onFileSelected((filePath) => {
        this.handleElectronFileSelect(filePath);
      });
    }
  }
  ...
  async saveCriteria() {
    ...
    if (window.electronAPI) {
      await window.electronAPI.setSetting('criteria', criteria);
    }
  ...
  async handleFileSelect(file) {
    ...
    try {
      ...
      if (window.electronAPI && file.path) {
        await window.electronAPI.addRecentFile(file.path);
      }
  ...
  applyValidationStyling(validationResults) {
    ...
    Object.entries(validationResults).forEach(([key, validation]) => {
      const row = rows[key];
      if (row) {
        row.className = `result-item ${validation.status}`;
      }
    });
  }
